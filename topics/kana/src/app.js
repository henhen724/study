...
const TOPIC_ID = "kana-en-loanwords";
...
function pickScriptFor(mode) {
  if (mode === "mixed") return Math.random() < 0.5 ? "katakana" : "hiragana";
  return mode;
}

// Ignore spacing/punctuation differences ("super market" vs "supermarket")
// and tolerate a single-character typo on longer words.
...
class Session {
  constructor(deck, scriptMode, progress) {
  ...
  nextCard() {
    if (this.current) {
      // current card wasn't resolved (shouldn't normally happen)
  ...
  buildQuestion(card) {
    ...
    const script = pickScriptFor(this.scriptMode);
    const kanaText = card.word[script];
    const useTyped = card.streak > 0 || level >= TYPED_FROM_LEVEL;
    let choices = null;
    if (!useTyped) {
      const pool = shuffle(
        this.deck.filter((w) => w.english !== card.word.english)
      ).slice(0, 3);
...
function renderQuestion(q) {
  ...
  el.prompt.textContent = q.kanaText;
  el.romajiHint.textContent = "";
  el.romajiHint.classList.add("hidden");
...
function handleAnswer(answer) {
  ...
  el.romajiHint.textContent = `${q.kanaText}  →  ${q.romaji}`;
  el.romajiHint.classList.remove("hidden");
...
async function endSession() {
  ...
  el.summaryStats.innerHTML = `
    <p><strong>${session.total}</strong> words mastered this round</p>
